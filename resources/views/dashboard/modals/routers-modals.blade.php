{{-- ══════════════════════════════════════════════════════════════
     MODAL 1 : CRÉATION / ÉDITION — ROUTEUR
     (déjà présent dans routers.blade.php — conservé ici pour référence)
     ══════════════════════════════════════════════════════════════ --}}
...
{{-- ══════════════════════════════════════════════════════════════
     MODAL 2 : TEST DE CONNECTIVITÉ — TOUS ÉQUIPEMENTS
     ✅ ID unique : testConnectivityModal
     ✅ Fonctionne pour switch, router ET firewall via modalData.type
     ══════════════════════════════════════════════════════════════ --}}
<div id="testConnectivityModal"
    ...
    <div style="background: white; border-radius: var(--border-radius-lg);
        ...
        {{-- HEADER — couleur dynamique selon le type d'équipement --}}
        <div :style="{
...
{{-- ══════════════════════════════════════════════════════════════
     MODAL 3 : MISE À JOUR DES INTERFACES — ROUTEUR
     ══════════════════════════════════════════════════════════════ --}}
            ...
            <div style="background: #f8fafc; padding: 20px;
                ...
                <h4 style="color:#10b981; margin: 0 0 12px; display:flex; align-items:center; gap:8px;">
                    ...
                    <span style="font-weight:400; font-size:.8rem; color:var(--text-light);">
                        (JSON — interface, status, ip, description)
                    </span>
                </h4>
...
{{-- ══════════════════════════════════════════════════════════════
     MODAL 4 : DÉTAILS — ROUTEUR
     ✅ viewItem('routers', rt.id) → currentModal = 'view', modalData.type = 'router'
     ══════════════════════════════════════════════════════════════ --}}
        ...
        <div style="padding: 20px 24px; border-top: 2px solid var(--border-color);
            ...
            <div style="display:flex; gap:10px;">
                <button class="btn btn-outline btn-sm"
                        @click="testConnectivity(modalData.type, modalData.item?.id); closeModal('viewEquipmentModal')">
                ...
                <template x-if="modalData.type === 'router'">
                    <button class="btn btn-outline btn-sm"
                            @click="updateInterfaces(modalData.item?.id); closeModal('viewEquipmentModal')">
                        <i class="fas fa-ethernet"></i> Interfaces
                    </button>
                </template>
                <template x-if="modalData.type === 'firewall'">
                    <button class="btn btn-outline btn-sm"
                            @click="updateSecurityPolicies(modalData.item?.id); closeModal('viewEquipmentModal')">
