<div class="fade-in">
    ...
    <section class="charts-section">
        <div class="chart-card">
            ...
            <div class="chart-container"><canvas id="deviceDistributionChart"></canvas></div>
        </div>
        <div class="chart-card">
            <div class="chart-header">
                    ...
                    <template x-if="firewalls && firewalls.length > 0">
                        <div>
                            <template x-for="firewall in firewalls.slice(0, 3)" :key="firewall.id">
                                {{--
                                    ✅ FIX 3a — @click appelle viewEquipmentDetails(type, item)
                                    en passant l'OBJET directement (pas l'id).
                                    viewEquipmentDetails stocke { type, item } dans modalData
                                    et ouvre viewEquipmentModal — cohérent avec le modal ci-dessous.
                                --}}
                                <div style="padding: 12px; border: 1px solid var(--border-color); cursor: pointer; transition: all 0.2s; border-radius: 8px; margin-bottom: 12px; background: white;" 
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                        ...
                                        <span class="status-badge" 
                                              :class="firewall.status === 'active' ? 'status-active' : 'status-danger'" 
                                              style="font-size: 0.7rem;">
                                            <span x-text="firewall.status === 'active' ? 'Actif' : 'Inactif'"></span>
                                        </span>
                                    </div>
                            ...
                            <template x-for="router in routers.slice(0, 3)" :key="router.id">
                                <div style="padding: 12px; border: 1px solid var(--border-color); cursor: pointer; transition: all 0.2s; border-radius: 8px; margin-bottom: 12px; background: white;" 
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                        ...
                                        {{-- ✅ status STRING (active|danger) --}}
                                        <span class="status-badge" 
                                              :class="router.status === 'active' ? 'status-active' : 'status-danger'" 
                                              style="font-size: 0.7rem;">
                                            <span x-text="router.status === 'active' ? 'Actif' : 'Inactif'"></span>
                                        </span>
                                    </div>
                            ...
                            <template x-for="sw in switches.slice(0, 3)" :key="sw.id">
                                <div style="padding: 12px; border: 1px solid var(--border-color); cursor: pointer; transition: all 0.2s; border-radius: 8px; margin-bottom: 12px; background: white;" 
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                        ...
                                        {{-- ✅ status STRING (active|warning|danger) --}}
                                        <span class="status-badge" 
                                              :class="{
                                              ...
                                              style="font-size: 0.7rem;">
                                            <span x-text="sw.status === 'active' ? 'Actif' : sw.status === 'warning' ? 'Avert.' : 'Critique'"></span>
                                        </span>
                                    </div>
...
{{--
    ✅ FIX 3b — Modal de détails
    Corrections apportées :
    1. ID du modal : "viewEquipmentModal" (cohérent avec showModal('viewEquipmentModal') dans viewEquipmentDetails())
    2. Bouton ✕ header : closeModal('viewEquipmentModal')     ← était déjà correct
    3. Bouton Fermer footer : closeModal('viewEquipmentModal') ← CORRIGÉ (était 'equipmentDetailsModal')
    4. x-html appelle renderEquipmentDetails() qui retourne '' si item absent → aucun flash
--}}
<div id="viewEquipmentModal"
    ...
    <div style="background: white; border-radius: var(--border-radius-lg);
        ...
        <!-- Contenu — renderEquipmentDetails() retourne '' si item=null → aucun flash -->
        ...
        <div style="padding: 20px 24px; border-top: 2px solid var(--border-color);
            ...
            {{-- ✅ FIX 3b — ID corrigé : 'viewEquipmentModal' (était 'equipmentDetailsModal') --}}
